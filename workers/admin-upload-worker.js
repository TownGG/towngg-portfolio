...
export default {
  async fetch(request, env) {
    ...
    if (request.method !== "POST" || (!isAuthCheck && !isGalleryUpload && !isGalleryList && !isGalleryDelete && !isCommunityRoute && !isMessageBoardRoute)) {
...
function flattenDiscussionComment(comment, parentId = "") {
  ...
  return [
    base,
    ...replies.flatMap((reply) => flattenDiscussionComment(reply, comment.id))
  ];
}
