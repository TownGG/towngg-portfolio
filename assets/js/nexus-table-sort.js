(() => {
  const TABLE_SELECTOR = '[data-dashboard-table]';
  ...
  function setHeaderState(headerRow, activeIndex, direction) {
    [...headerRow.children].forEach((header, index) => {
      ...
      header.querySelector('[data-nexus-table-sort]')?.setAttribute('aria-sort', isActive ? direction : 'none');
    });
  }
  ...
  function enhanceTable() {
    ...
    table.dataset.sortableTable = 'nexus';

    [...headerRow.children].forEach((header, index) => {
      const label = header.textContent.trim();
      const type = columnType(label);
      if (!type || header.querySelector('[data-nexus-table-sort]')) return;

      header.innerHTML = `<button type="button" class="dashboard-table-sort" data-nexus-table-sort="${normalize(label)}" aria-sort="none">${label}</button>`;
      header.querySelector('[data-nexus-table-sort]').addEventListener('click', () => {
